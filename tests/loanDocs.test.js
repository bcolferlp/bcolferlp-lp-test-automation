  ...
  describe.skip('DocuSign Test Suite', () => {
    ...
    each(singleBorrData).test.skip(
      ...
      async ({ loanType, productType, clientId, firstName, lastName, street, state, email, spokenLanguage, source, salesRepEmail }, done) => {
        ...
        //validate documents in email and sign docs
        emailBorrower = new EmailLoanBorrower();
        expect(emailBorrower.checkLoanIdinMessage()).toBeTruthy
        let linkToSignDocs = emailBorrower.getLinkfromBody()
        let signDocs = new signDocs(linkToSignDocs)
        let signDocsCompleted = signDocs.sign()
        expect(signDocsCompleted).toBeTruthy
        ...
        comparePDFs = new LoanDocsCompare(testFile, expectedFile)
        resultComparison = comparePDFs.compare()
        expect(resultComparison).toBeTruthy
        done();
      },
      300000
    );
