...
describe('Email', () => {
  describe.only('Check Inbox', () => {
    ...
    describe.only.each([IP431File[0]])('IP-431', record => {
      // Solar Financing Decision email
      // console.log(record);
      test.only.each(['english', 'spanish'])(`Validate %s ${record.scenario} with ${record.stips} for a ${record.type}`, async language => {
